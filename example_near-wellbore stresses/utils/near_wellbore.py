...
def stress_direction_thetas(row) -> dict:
    """θ positions of the SHmax- and Shmin-facing wall points.

    θ = 0° (top of hole) lies in the vertical plane through the borehole axis,
    whose horizontal bearing is the borehole azimuth. For a vertical hole the
    wall point at angle θ therefore faces the bearing (borehole azimuth + θ),
    so SHmax sits at θ = SHmax azimuth − borehole azimuth. This is exact only
    while the hole is vertical: once it deviates the wall circle tilts and θ no
    longer maps onto a compass bearing, so the values are a guide there.
